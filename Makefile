...
override CFLAGS += -mthumb -mthumb-interwork -O$(O_LEVEL) -mabi=apcs-gnu -mtune=arm7tdmi -march=armv4t -Wno-pointer-to-int-cast -std=gnu17 -Werror -Wall -Wno-strict-aliasing -Wno-attribute-alias -Woverride-init -Wnonnull -Wenum-conversion -Wenum-compare -Wswitch
...
TEACHABLE_DEPS := $(ALL_LEARNABLES_JSON) $(LEARNSET_HELPERS_DIR)/make_teachables.py $(shell find data/ -type f -name '*.inc') $(INCLUDE_DIRS)/constants/tms_hms.h $(DATA_SRC_SUBDIR)/technique_manual.h $(INCLUDE_DIRS)/config/pokemon.h $(C_SUBDIR)/pokemon.c
