...
TILEMAPCOMP3 := $(TOOLS_DIR)/compressTilemap3/compressTilemap3$(EXE)
...
clean-assets:
	...
	find . \( -iname '*.1bpp' -o -iname '*.4bpp' -o -iname '*.8bpp' -o -iname '*.gbapal' -o -iname '*.lz' -o -iname '*.rl' -o -iname '*.tilemapcomp3' -o -iname '*.latfont' -o -iname '*.hwjpnfont' -o -iname '*.fwjpnfont' \) -exec rm {} +
...
%.lz:     %      ; $(GFX) $< $@
%.rl:     %      ; $(GFX) $< $@
%.tilemapcomp3: % $(TILEMAPCOMP3) ; $(TILEMAPCOMP3) $< $@
