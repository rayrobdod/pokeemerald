...
int main(int argc, char **argv)
{
    ...
    if (NULL == args.outputPath)
        FATAL_ERROR("Usage: compressFrit INPUT_PATH OUTPUT_PATH [options...]\n");
        ...
        case COMPRESS:
            {
                ...
                struct ShortArray round_trip = decompress16(output, VERBOSE_SILENT);
        ...
        case DECOMPRESS:
            {
                ...
                struct ShortArray output = decompress16(input, VERBOSE_DISASSEMBLE);
                free(input.buffer);
    ...
    case WORD_SIZE_8:
        ...
        case COMPRESS:
            {
                ...
                struct ByteArray round_trip = decompress8(output, VERBOSE_SILENT);
                for (int i = 0; i < input.size; i++)
                    if (input.buffer[i] != round_trip.buffer[i])
                        FATAL_ERROR("Mismatch: index %d, input %04x, roundtrip %04x\n", i, input.buffer[i], round_trip.buffer[i]);
        ...
        case DECOMPRESS:
            {
                struct ByteArray input = ReadWholeFileByte(args.inputPath);
                struct ByteArray output = decompress8(input, VERBOSE_DISASSEMBLE);
                free(input.buffer);
                WriteWholeFileByte(args.outputPath, output);
                free(output.buffer);
            }
