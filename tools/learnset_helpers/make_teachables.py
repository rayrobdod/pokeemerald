...
MANUAL_PAT = re.compile(r"\.move = (MOVE_.*),")
...
def extract_repo_manuals() -> typing.Generator[str, None, None]:
    """
    Yield MOVE constants assigned to the Technique Manual in the user's repo.
    """
    with open("./src/data/technique_manual.h", "r") as tms_fp:
        tms = tms_fp.read()
        match_it = MANUAL_PAT.finditer(tms)
        if not match_it:
            return
...
def prepare_output(all_learnables: dict[str, set[str]], tms: list[str], tutors: list[str], manuals: list[str], special_movesets, repo_teaching_types, header: str) -> str:
    """
    ...
    for species_data in repo_teaching_types:
        ...
        if teaching_type == "ALL_TEACHABLES":
            ...
            part3 = list(filter(lambda m: m not in special_movesets["signatureTeachables"], manuals))
        else:
            ...
            part3 = list(filter(lambda m: m in learnables, manuals))


        repo_species_teachables = part1 + part2 + part3
...
def prepare_header(h_align: int, tmshms: list[str], tutors: list[str], manuals: list[str], universals: list[str]) -> str:
    ...
    lines.extend([
        f"// {'*' * h_align} //",
        f"// {manual_title: <{h_align}} //",
    ])
    lines.extend([f"// - {move: <{h_align - 2}} //" for move in sorted(manuals)])
    lines.extend([
        f"// {'*' * h_align} //",
...
def main():
    ...
    repo_manuals = list(extract_repo_manuals())
    ...
    h_align = max(map(lambda move: len(move), chain(special_movesets["universalMoves"], repo_tms, repo_tutors, repo_manuals))) + 2
    header = prepare_header(h_align, repo_tms, repo_tutors, repo_manuals, special_movesets["universalMoves"])
    ...
    content = prepare_output(all_learnables, repo_tms, repo_tutors, repo_manuals, special_movesets, repo_teaching_types, header)
