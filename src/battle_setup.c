...
void SetMapVarsToTrainerA(void)
{
    ...
    {
        StringCopy(gSpeakerName, gTrainers[DIFFICULTY_NORMAL][TRAINER_BATTLE_PARAM.opponentA].trainerName);
    }
}
...
void SetMapVarsToTrainerB(void)
{
    ...
    {
        StringCopy(gSpeakerName, gTrainers[DIFFICULTY_NORMAL][TRAINER_BATTLE_PARAM.opponentB].trainerName);
    }
}
...
static const u8 *GetIntroSpeechOfApproachingTrainer(void)
{
    // I feel like adding `trainerNameA` and `trainerNameB` to `TRAINER_BATTLE_PARAM` would be preferred,
    // as opposed to peeking in `gTrainers`, especially for handling difficulties
    // but `TRAINER_BATTLE_PARAM` is used in the assembly and trying to merge that would probably have
    // less noticeable problems and merge conflicts than continuing to peek inside `gTrainers` 
    if (gApproachingTrainerId == 0)
    {
        StringCopy(gSpeakerName, gTrainers[DIFFICULTY_NORMAL][TRAINER_BATTLE_PARAM.opponentA].trainerName);
