...
static enum MoveEndResult MoveEndTmSeenTracker(struct BattleCalcValues *cv)
{
    enum Move originallyUsedMove = GetOriginallyUsedMove(gChosenMove);
    if (originallyUsedMove != MOVE_NONE)
        TmIncrementSeenStats(originallyUsedMove, gBattleMons[cv->battlerAtk].species);
...
static enum MoveEndResult (*const sMoveEndHandlers[])(struct BattleCalcValues *cv) =
{
    ...
    [MOVEEND_TM_SEEN_TRACKER] = MoveEndTmSeenTracker,
